...
): ExchangeDataResponseSymbolData[Field] | undefined {
  if (!(field in data)) {
      ...
      (resolve: () => void, reject: (error: Error) => void) => {
        this._requester
          ...
          .catch((reason?: string | Error) => {
            logMessage(
              `SymbolsStorage: Request data for exchange '${exchange}' failed, reason=${getErrorMessage(reason)}`,
            );
            resolve();
          });
      },
    ...
    try {
      ...
      for (; symbolIndex < symbolsCount; ++symbolIndex) {
        ...
        if (listedExchange !== undefined || tradedExchange !== undefined) {
          // eslint-disable-next-line no-console
          console.warn(
            'Starting from v30, both "exchange-listed" and "exchange-traded" fields are deprecated. Please use "exchange_listed_name" instead.',
          );
          fullName = tradedExchange + ':' + symbolName;
        }

        const exchangeListedName = extractField(
          data,
          'exchange_listed_name',
          symbolIndex,
        );
